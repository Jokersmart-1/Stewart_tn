...
| File | Nội dung |
|------|----------|
| [`README.md`](README.md) | Tổng quan hệ thống và các giai đoạn phát triển |
| [`system_architecture.md`](system_architecture.md) | Kiến trúc hệ thống, cơ chế bộ đệm kép, sơ đồ luồng, GPIO |
| [`01_hardware.md`](01_hardware.md) | Sơ đồ chân, phần cứng, motor drivers |
| [`02_clock_timer.md`](02_clock_timer.md) | Cấu hình clock 180MHz, TIM1 DDA 200kHz |
...
| **Phase 2** | TIM1 DDA — test bắn xung STEP1, STEP2 | ✅ Hoàn thành |
| **Phase 3** | Mở rộng DDA đồng bộ 6 trục với bộ đệm kép (5ms) | ✅ Hoàn thành |
| **Phase 4** | USB Serial — giao tiếp truyền nhận lệnh di chuyển | ✅ Hoàn thành |
| **Phase 5** | SDIO — nạp firmware qua MicroSD | ⏳ Chưa bắt đầu |
| **Phase 6** | Tích hợp & kiểm tra toàn hệ thống | 🔄 Đang tích hợp |
...
| **Timer DDA** | TIM1 @ 200 kHz interrupt (DMA Trigger) |
| **Bộ đệm kép DDA** | 2 × 5 ms (1000 ticks) cho 6 trục (Port F, G, C BSRR) |
| **Số trục** | 6 (STEP0–STEP5) |
| **Giao tiếp Raspi** | USB OTG FS VCP (PA11/PA12) |
| **Nạp firmware** | SDIO MicroSD (4-bit wide bus) |
| **Step/Dir Output** | GPIO Push-Pull (Active-Low cho STEP) |
...
- [`Core/Src/main.c`](../Core/Src/main.c) — Khởi tạo hệ thống, cấu hình clock và gọi vòng lặp DDA
- [`Core/Src/main_pingpong.c`](../Core/Src/main_pingpong.c) — Trình điều khiển DDA 6 trục đồng bộ, bộ đệm kép (5ms)
- [`Core/Inc/main.h`](../Core/Inc/main.h) — Định nghĩa tất cả chân GPIO và cấu trúc chuyển động
- [`Core/Src/stm32f4xx_it.c`](../Core/Src/stm32f4xx_it.c) — Ngắt DMA2_Stream5 hoán đổi bộ đệm kép và ngắt USB OTG
- [`Core/Src/gpio.c`](../Core/Src/gpio.c) — Khởi tạo GPIO
- [`Core/Src/sdio.c`](../Core/Src/sdio.c) — Giao tiếp SDIO MicroSD
- [`Core/Src/usb_otg.c`](../Core/Src/usb_otg.c) — Khởi tạo ngoại vi USB OTG FS
